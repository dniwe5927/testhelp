...
Global $iW = 600, $iH = 400, $iT = 52, $iB = 52, $iLeftWidth = 200, $iGap = 10, $hMainGUI
...
Local $iLinks = 5
Local $sMainGuiTitle = "'Ф.О.Н' helpper"
Local $sHeader = "Ну-ка скажи мне что у тебя не работает?"
Local $sFooter = "create by BitBlame"
...
GUICtrlCreateIcon("wf3l2t.ico", -131, 8, 8, 32, 32)
GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
...
$aLink[1] = _AddNewLink("не работает принтер")
$aLink[2] = _AddNewLink("не работает се", -167)
$aLink[3] = _AddNewLink("не работает все", -222)
$aLink[4] = _AddNewLink("не работает ничего", -22)
;and the corresponding GUI's
$aPanel[1] = _AddNewPanel("какой принтер не работает")
$aPanel[2] = _AddNewPanel("не работает се")
$aPanel[3] = _AddNewPanel("не работает все")
$aPanel[4] = _AddNewPanel("не работает ничего")

;add some controls to the panels
_AddControlsToPanel($aPanel[1])
GUICtrlCreateGroup("", 8, 35, 129, 90)
...
Local $hButton1 = GUICtrlCreateButton("починить", 200, 33, 75, 25)
GUICtrlSetResizing(-1, $GUI_DOCKTOP + $GUI_DOCKLEFT + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
...
Next
Case $hFooter
ShellExecute("skype:tehkazan?chat")
EndSwitch
Case $aPanel[2]
Switch $nMsg[0]
Case $hButton1
